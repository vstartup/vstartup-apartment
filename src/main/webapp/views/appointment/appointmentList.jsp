...
<body >
	<form ="search_editfrom" action="${ctx}/appointment/preEdit" class="form-inline" role="form">
		<input type="hidden" class=search_id name="search_id value=">
	</form>
	<form id="search_deletefrom" action="${ctx}/appointment/doDelete" class="form-inline" role="form">
		<input type="hidden" class="search_id" name="search_id" value="">
	</form>
	...
	<div class=" box-content row col-md-12">
	    ...
	    <table id="appointment_table" class="table table-striped table-bordered  responsive">
	    	<thead>
			    <tr>
			        <th>客户编号</th>
			        <th>电话</th>
		    ...
		    <tbody>
			    <c:forEach var="instance" items="${page.content}" varStatus="status">
			    	<tr>
				        ...
				        <td class="center"><fmt:formatDate value="${instance.appointTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/></td>
				        <td class="center"><fmt:formatDate value="${instance.applyTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/></td>
				        <td class="center" style="padding-top:16px">
				        	...
				        	<c:if test="${instance.statusCode==3}">
				        		<span class="label-warning label label-default">${instance.statusName}</span>
				        	</c:if>
				        </td>
				        <td class="center">${instance.remark}</td>
				        <td class="center">
				        	<!-- 
...
require(["dojo/dom","dojo/_base/xhr",
	...
	showEditWindow = function(){
		...
		popupWindow('CustomerEditWin',"search_editfrom","订单修改",380,400,true);
	}
